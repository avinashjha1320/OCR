...
# Define the regex patterns for your four cases
unique_id_pattern = re.compile(r'\b[A-Z]{2,3}-[A-Za-z0-9]{4,9}[-#&*+\w]*\b')
product_description_pattern = re.compile(r'[A-Za-z0-9 ]{10,}')
product_price_pattern = re.compile(r'\b\d{3,5}\b')
dimension_pattern = re.compile(r'\b\d{2,3}\s*X\s*\d{2,3}\b')
...
column_patterns = [('Unique ID', unique_id_pattern), ('Product Description', product_description_pattern), ('Product Price', product_price_pattern), ('Dimension', dimension_pattern)]
